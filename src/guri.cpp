...
const std::string GUri::K_Self= "_$";


static bool isElemName(std::string& aSrc)
{
    bool res = true;
    if (aSrc.size() > 0) {
...
void GUri::parse(const std::string& aSrc)
{
    bool res = true;
    mElems.clear();
    if (aSrc.size() > 0) {
	size_t beg = 0, pos = 0;
	while (pos !=std::string::npos && res) {
	    pos = aSrc.find_first_of(KSep, beg); 
	    std::string elem = aSrc.substr(beg, pos - beg);
	    if (beg == 0) {
	    ...
	    beg = (pos == std::string::npos) ? std::string::npos : pos + 1;
	}
    } else {
...
{
    return mElems == s.mElems && mValid == s.mValid;
}

bool GUri::operator<(const GUri& aSrc) const
{
    bool res = mValid && mElems.size() < aSrc.mElems.size();
...
std::string GUri::toString() const
{
    std::string res;
    bool first = true;
